...
bool is_alpha(int a) { return (96 < a && a < 123); }
...
bool es_func(string line) {
  ...
  for (int x : line) {
    ...
    else if (!nombre && is_alpha(x))
      nombre = true;
    else if (nombre && !parder && x == 40)
      parizq = true;
    else if (nombre && parizq && !parder && x == 41)
      parder = true;
    else
      continue;
  }
...
int encontrar_siguiente_simple(int i) {
  for (int j = i + 1; j < lineas.size(); j++) {
    ...
    if (contar_indent(line) <= contar_indent(lineas[i]) &&
        !es_else(line)) { // cualquier caso posible
      return j;
    }
  }
...
void conectar_hijos(int a, int b, int c) {
  ...
  if (es_def(lineas[k]) || es_func(lineas[k])) {
    ...
    if (!exis_else(k)) {
      CFG[nodo_linea[k]][nodo_linea[c]] = 1; // conectar nodo con el nodo
                                             // deseado
      conectar_hijos(ult_linea(nodo_linea[k] + 1), -1, c);
    } else {
      CFG[nodo_linea[k]][nodo_linea[encontrar_siguiente_else(k)]] =
          1; // conectar con el hijo de else
      conectar_hijos(ult_linea(nodo_linea[k] + 1), encontrar_siguiente_else(k),
                     c);
    }
...
//---------------------------------------------------------------------
string ifwhile_true(
    string line, bool t_f = false,
    bool true_ = false) { // funcion solo para dejar string en el formato pedido
  string nueva_linea;
  if (es_while(line)) {
...
vector<int> variable_utl(string line) {
  ...
  if (es_if(line) || es_while(line)) {
    ...
    while (*itr != 61)
      itr++; // avanzar hasta llegar al igual
    while (itr != line.end()) {
      if (is_alpha(*itr))
        variables.push_back(*itr);
      itr++;
...
void variables_indefinidas(int bloque_, vector<int> nodos_visitados,
                           set<int> variables_, vector<string> camino) {
  if (bloque_ == CFG.size() - 1)
    return; // linea == "Fin"
  string line;
  vector<int> var_;
  int var;
  for (int i = 0; i < nodo_linea.size(); i++) {
    if (nodo_linea[i] == bloque_) { // pertenece al bloque
      ...
      for (int a : var_) {
        if (variables_.find(a) ==
            variables_.end()) { // si alguan variable no fue definida antes
          string c = camino[camino.size() - 1];
          if (es_if(camino[camino.size() - 1]) ||
              es_while(camino[camino.size() - 1])) {
  ...
  for (int j = 0; j < CFG.size(); j++) {
    if (CFG[bloque_][j] && !nodos_visitados[j]) {
      ...
      if (es_if(camino[camino.size() - 1]) ||
        ...
        camino[camino.size() - 1] =
            ifwhile_true(camino[camino.size() - 1], true, t_f);
      }
...
int main() {
  string nombre;
  cout << "Introduzca './(nombre_archivo).txt':" << endl;
  cin >> nombre;
  ...
  CFG.assign(Bloques.size(),
             vector<int>(Bloques.size(), 0)); // Matriz de adyacencia
  for (int i = 0; i < Bloques.size() - 1;
       i++) { // menos Fin (no conecta con nada)
    ...
    // los bloques que terminan en instrucciones simples o definiciones siempre
    // conectan solo con 1
    ...
    if (es_while(linea) && !esta_conectado(i, 2)) {
      CFG[i][i + 1] = 1;
      // si leemos un while
      //  --> conectamos el while con el siguiente y con el siguiente fuera del
      //  while (funcion encontrar siguiente no contenido y siguiente simple)
      ...
      //     el ultimo debe tener igual indentacion que el primer bloque despues
      //     del while si es una instruccion o un while la conectamos
      //     simplemente si es un if, conectamos sus nodos finales con el while
      //          --> si es un if simple conectar la ultima instruccion del if
      //          con el while
      //          --> si es un if_else conectar las 2 ultimas instrucciones con
      //          el while
      conectar_hijos(ult_linea(i + 1), -1, j);
    } else if (es_if(linea) && !esta_conectado(i, 2)) {
      // si el bloque termina con una condicion if
      //  sin else:
      //  --> conectar el bloque del if con el siguiente y con el siguiente que
      //  tenga igual o menor indentacion con else:
      //  --> conectar el bloque con el siguiente y el siguiente despues del
      //  else
      //  --> la ultima instruccion contenida en el if debe estar conectada con
      //  lo mismo que se conecta la instruccion final del else
      CFG[i][i + 1] = 1;
      if (!exis_else(j)) { // si no existe un else para el if
        ...
        CFG[i][nodo_linea[nodo_]] = 1;
        conectar_hijos(ult_linea(i + 1), ult_linea(nodo_linea[nodo_]),
                       encontrar_siguiente_simple(j));
      }
      ...
      // si el bloque actual termina con igual indentacion con la que comienza
      // el bloque siguiente entonces conectar directo
